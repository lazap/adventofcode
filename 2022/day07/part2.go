...
func main() {
	...
	diskSpaceUsed := rootDir.calculateSize()
	freeSpaceOnDisk := 70000000 - diskSpaceUsed
	spaceToBeFreed := 30000000 - freeSpaceOnDisk

	result := &Result{
		directorySizes: make([]int, 0),
	}
	findAnswer(rootDir, spaceToBeFreed, result)

	min := result.directorySizes[0]
	for _, size := range result.directorySizes {
		if size < min {
			min = size
		}
	}
...
func findAnswer(dir *Directory, spaceToBeFreed int, result *Result) {
	currentDirSize := dir.calculateSize()
	if currentDirSize >= spaceToBeFreed {
		result.directorySizes = append(result.directorySizes, currentDirSize)
	}
	for _, item := range dir.items {
		directory, ok := item.(*Directory)
		if ok {
			findAnswer(directory, spaceToBeFreed, result)
		}
	}
}
